...
public class RemoteCASTickerConsumerHomeEventImpl extends ClientBOHome implements IECRemoteCASTickerConsumerHome
{
    private RemoteCASTickerEventConsumerInterceptor consumerProxy;
    private RemoteCASTickerEventConsumerImpl event;
        ...
        throws Exception
    {
        ...
        String interfaceRepId = com.cboe.idl.events.RemoteCASTickerEventConsumerHelper.id();
    ...
    public void clientInitialize()
    {
        ...
        RemoteCASTickerConsumerIECImpl consumer = new RemoteCASTickerConsumerIECImpl();
        consumer.create(String.valueOf(consumer.hashCode()));

        //Every BObject must be added to the container.
        addToContainer(consumer);
        consumerProxy = new RemoteCASTickerEventConsumerInterceptor(consumer);
        if( getInstrumentationEnablementProperty())
        {
            consumerProxy.startInstrumentation(getInstrumentationProperty());
        }
        event = new RemoteCASTickerEventConsumerImpl(consumerProxy);
    }
    ...
    private String getParmName(ChannelKey channelKey)
    {
        ...
        switch(channelKey.channelType)
        {
            case ChannelType.SUBSCRIBE_TICKER_BY_CLASS_V2:
                name.append(key).append(" in $.subscribeTickerForClassV2.routingParameters.groups");
                return name.toString();
            
            case ChannelType.SUBSCRIBE_LARGE_TRADE_LAST_SALE_BY_CLASS:
                name.append(key).append(" in $.subscribeLargeTradeLastSaleForClass.routingParameters.groups");
                return name.toString();

            case ChannelType.SUBSCRIBE_TICKER_BY_PRODUCT_V2:
                name.append(key).append(" in $.subscribeTickerForProductV2.routingParameters.groups");
                return name.toString();

            case ChannelType.UNSUBSCRIBE_TICKER_BY_CLASS_V2:
                name.append(key).append(" in $.unsubscribeTickerForClassV2.routingParameters.groups");
                return name.toString();

            case ChannelType.UNSUBSCRIBE_LARGE_TRADE_LAST_SALE_BY_CLASS:
                name.append(key).append(" in $.unsubscribeLargeTradeLastSaleForClass.routingParameters.groups");
                return name.toString();

            case ChannelType.UNSUBSCRIBE_TICKER_BY_PRODUCT_V2:
                name.append(key).append(" in $.unsubscribeTickerForProductV2.routingParameters.groups");
                return name.toString();

            case ChannelType.SUBSCRIBE_TICKER_BY_PRODUCT:
                name.append(key).append(" in $.subscribeTickerForProduct.routingParameters.groups");
                return name.toString();

            case ChannelType.UNSUBSCRIBE_TICKER_BY_PRODUCT:
                name.append(key).append(" in $.unsubscribeTickerForProduct.routingParameters.groups");
                return name.toString();
    ...
    {
        String keyString = channelKey.toString();
        StringBuilder adding = new StringBuilder(keyString.length() + 30);
        adding.append("Attempting to add filter for ").append(keyString);
        Log.information(this, adding.toString());
        ...
        throws SystemException
    {
        ...
        if(Log.isDebugOn())
        {
            Log.debug(this, "add constraint: channelType=" + channelKey.channelType + " channelKey:" + channelKey.key + " constraint=" + constraintString);
        }
