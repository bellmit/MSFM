...
public class RemoteCASNBBOConsumerHomeEventImpl extends ClientBOHome implements IECRemoteCASNBBOConsumerHome
{
    private RemoteCASNBBOEventConsumerInterceptor consumerProxy;
    private RemoteCASNBBOEventConsumerImpl event;
        ...
        throws Exception
    {
        ...
        String interfaceRepId = com.cboe.idl.events.RemoteCASNBBOEventConsumerHelper.id();
    ...
    public void clientInitialize()
    {
        ...
        RemoteCASNBBOConsumerIECImpl consumer = new RemoteCASNBBOConsumerIECImpl();
        consumer.create(String.valueOf(consumer.hashCode()));

        //Every BObject must be added to the container.
        addToContainer(consumer);
        consumerProxy = new RemoteCASNBBOEventConsumerInterceptor(consumer);
        if(getInstrumentationEnablementProperty())
        {
            consumerProxy.startInstrumentation(getInstrumentationProperty());
        }
        event = new RemoteCASNBBOEventConsumerImpl(consumerProxy);
    }
    ...
    private String getParmName(ChannelKey channelKey)
    {
        ...
        StringBuilder name = new StringBuilder(key.length()+60);

        switch(channelKey.channelType)
        {
            case ChannelType.SUBSCRIBE_NBBO_BY_CLASS:
                name.append(key).append(" in $.subscribeNBBOForClass.routingParameters.groups");
                return name.toString();

            case ChannelType.SUBSCRIBE_NBBO_BY_PRODUCT:
                name.append(key).append(" in $.subscribeNBBOForProduct.routingParameters.groups");
                return name.toString();

            case ChannelType.UNSUBSCRIBE_NBBO_BY_CLASS:
                name.append(key).append(" in $.unsubscribeNBBOForClass.routingParameters.groups");
                return name.toString();

            case ChannelType.UNSUBSCRIBE_NBBO_BY_PRODUCT:
                name.append(key).append(" in $.unsubscribeNBBOForProduct.routingParameters.groups");
                return name.toString();

            case ChannelType.SUBSCRIBE_NBBO_BY_CLASS_V2:
                name.append(key).append(" in $.subscribeNBBOForClassV2.routingParameters.groups");
                return name.toString();

            case ChannelType.SUBSCRIBE_NBBO_BY_PRODUCT_V2:
                name.append(key).append(" in $.subscribeNBBOForProductV2.routingParameters.groups");
                return name.toString();

            case ChannelType.UNSUBSCRIBE_NBBO_BY_CLASS_V2:
                name.append(key).append(" in $.unsubscribeNBBOForClassV2.routingParameters.groups");
                return name.toString();

            case ChannelType.UNSUBSCRIBE_NBBO_BY_PRODUCT_V2:
                name.append(key).append(" in $.unsubscribeNBBOForProductV2.routingParameters.groups");
                return name.toString();
