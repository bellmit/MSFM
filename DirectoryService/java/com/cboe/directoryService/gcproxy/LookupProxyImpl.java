// Source file: L:/DirectoryService/java/com/cboe/directoryService/gcproxy/LookupProxyImpl.java
...
public class LookupProxyImpl extends  POA_Lookup{
	...
	*/
	private Lookup orcl_actual_Lookup;
	private Lookup ldap_actual_Lookup;
	...
	*/
	...
	public LookupProxyImpl() {
    	...
    	public LookupProxyImpl(java.util.Properties props, POA aPOA,Lookup orclaLookup,Lookup ldapaLookup)
	{
	   initialize(props);
	   poa = aPOA;
	   orcl_actual_Lookup = orclaLookup;	
	   ldap_actual_Lookup = ldapaLookup;	
	}
  ...
  public synchronized void query(String serviceType, String constr, String pref, Policy[] policies, SpecifiedProps desired_props,
	...
	{
      Logger.debug(this.getClass().getName() + " in the query method of LookupImpl of the proxy");
		SpecifiedProps specProps = new SpecifiedProps(HowManyProps.all);

                try
                {
		   orcl_actual_Lookup.query(serviceType, constr, pref, policies, specProps, how_many, offers, offer_itr, limits_applied);
                }
                ...
                {
                    Logger.debug(this.getClass().getName() + " COMM_FAILURE Exception: " +te.toString());
                    Logger.debug(this.getClass().getName() + " Shutting down GC Trader Proxy...");
                   orb.shutdown(true);
                   System.exit(1);
                }
                ...
                {
                    Logger.debug(this.getClass().getName() + " COMM_FAILURE Exception: " +oe.toString());
                    Logger.debug(this.getClass().getName() + " Shutting down GC Trader Proxy...");
                   orb.shutdown(true);
                   System.exit(1);
                }
                ...
                {
                    Logger.debug(this.getClass().getName() + " SystemException : " +corba_exception);
                   try
                   {
                       ldap_actual_Lookup.query(serviceType, constr, pref, policies, specProps, how_many, offers, offer_itr, limits_applied);
                   }
                   catch(org.omg.CORBA.COMM_FAILURE cf1)
                   {Logger.debug(this.getClass().getName() + " Shutting down GC Trader Proxy.. exception:" + cf1.toString());
                   ...
                   {
                       Logger.debug(this.getClass().getName() + " Shutting down GC Trader Proxy.. exception:" + te1.toString());

                       // Shudown the ORB
                       orb.shutdown(true);
                       System.exit(1);
                   }
                   catch(org.omg.CORBA.OBJECT_NOT_EXIST oe1)
                   {
                       Logger.debug(this.getClass().getName() + " Shutting down GC Trader Proxy.. exception:" + oe1.toString());
                   ...
                   {
                       Logger.debug(this.getClass().getName() + " Shutting down GC Trader Proxy.. exception:" + corba_exception1.toString());
