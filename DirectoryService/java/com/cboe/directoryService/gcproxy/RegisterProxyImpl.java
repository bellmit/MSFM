...
extends POA_Register
{
	...
	*/
	...
	private Register orcl_actual_Register; 
	private Register ldap_actual_Register; 
    ...
    */
	public RegisterProxyImpl( Properties props, POA poa_p, Register orclaRegister, Register ldapaRegister)
	{

       		initialize( props);
       		poa = poa_p;
		orcl_actual_Register = orclaRegister;
		ldap_actual_Register = ldapaRegister;
    ...
    */
	...
	PropertyTypeMismatch, ReadonlyDynamicProperty, MissingMandatoryProperty, DuplicatePropertyName, org.omg.CosTrading.RegisterPackage.InterfaceTypeMismatch
	{
	    String offerId = null;
            try
            {
                offerId = orcl_actual_Register.export(reference,type, properties);
                offerId = ldap_actual_Register.export(reference,type, properties); 
            }
	...
	*/
	...
	throws IllegalOfferId, UnknownOfferId,org.omg.CosTrading.RegisterPackage.ProxyOfferId
	{
            try
            {
		orcl_actual_Register.withdraw(id);	
                ldap_actual_Register.withdraw(id);
            }
            catch(org.omg.CORBA.COMM_FAILURE cf)
            {
                Logger.debug(this.getClass().getName() + " COMM_FAILURE Exception: " +cf.toString());
	...
	throws IllegalServiceType, UnknownServiceType, IllegalConstraint, NoMatchingOffers 
        {
            try
            {
		orcl_actual_Register.withdraw_using_constraint(type,constr);
		ldap_actual_Register.withdraw_using_constraint(type,constr);
            }
            catch(org.omg.CORBA.COMM_FAILURE cf)
            {
                Logger.debug(this.getClass().getName() + " COMM_FAILURE Exception: " +cf.toString());
	...
	*/
	...
	throws IllegalOfferId, UnknownOfferId,org.omg.CosTrading.RegisterPackage.ProxyOfferId
	{
            ...
            try
            {
		offerInfo = orcl_actual_Register.describe(id);
            }
            catch(org.omg.CORBA.COMM_FAILURE cf)
            {
                Logger.debug(this.getClass().getName() + " COMM_FAILURE Exception: " +cf.toString());
            ...
            catch(org.omg.CORBA.SystemException corba_exception)
            {
                ...
                try
                {
                    offerInfo = ldap_actual_Register.describe(id);
                }
    ...
    */
	...
	MandatoryProperty, ReadonlyProperty, DuplicatePropertyName, org.omg.CosTrading.RegisterPackage.ProxyOfferId, org.omg.CosTrading.NotImplemented, org.omg.CosTrading.ReadonlyDynamicProperty
	{
            try
            {
		orcl_actual_Register.modify(id,del_list,modify_list);
		ldap_actual_Register.modify(id,del_list,modify_list);
            }
            catch(org.omg.CORBA.COMM_FAILURE cf)
            {
                Logger.debug(this.getClass().getName() + " COMM_FAILURE Exception: " +cf.toString());
