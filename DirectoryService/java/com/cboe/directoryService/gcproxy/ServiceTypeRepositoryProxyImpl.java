...
extends POA_ServiceTypeRepository
{
	...
	*/
	private ServiceTypeRepository orcl_actual_ServiceTypeRepository; 
	private ServiceTypeRepository ldap_actual_ServiceTypeRepository; 
	...
	*/
	...
	public ServiceTypeRepositoryProxyImpl( Properties props, POA poa_p,ServiceTypeRepository orclaServiceTypeRepository,ServiceTypeRepository ldapaServiceTypeRepository)
	{
	   initialize(props);
	   poa = poa_p;
	   orcl_actual_ServiceTypeRepository = orclaServiceTypeRepository;	
	   ldap_actual_ServiceTypeRepository = ldapaServiceTypeRepository;	
	}
	...
	*/
	...
	{
            IncarnationNumber inCar = null;
            try
            {
	       inCar = orcl_actual_ServiceTypeRepository.add_type(name,if_name,props,super_types);
               inCar = ldap_actual_ServiceTypeRepository.add_type(name,if_name,props,super_types);
            }
	...
	*/
	...
	throws IllegalServiceType, UnknownServiceType, HasSubTypes
	{
            try
            {
	       orcl_actual_ServiceTypeRepository.remove_type(name);
	       ldap_actual_ServiceTypeRepository.remove_type(name);
            }
            catch(org.omg.CORBA.COMM_FAILURE cf)
            {
                Logger.debug(this.getClass().getName() + " COMM_FAILURE Exception: " +cf.toString());
	...
	*/
	public synchronized String[] list_types(SpecifiedServiceTypes which_types)
	    ...
	    Logger.debug(this.getClass().getName() + "  the type is " + which_types.discriminator());
            ...
            try
            {
	        typeList = orcl_actual_ServiceTypeRepository.list_types(specifiedTypes);
            }
            catch(org.omg.CORBA.COMM_FAILURE cf)
            {
                Logger.debug(this.getClass().getName() + " COMM_FAILURE Exception: " +cf.toString());
            ...
            catch(org.omg.CORBA.SystemException corba_exception)
            {
                ...
                try
                {
                    typeList = ldap_actual_ServiceTypeRepository.list_types(specifiedTypes);   
                }
	...
	*/
	...
	{
            TypeStruct descType  = null;
            try
            {
        	descType = orcl_actual_ServiceTypeRepository.describe_type(name);
            }
            catch(org.omg.CORBA.COMM_FAILURE cf)
            {
                Logger.debug(this.getClass().getName() + " COMM_FAILURE Exception: " +cf.toString());
            ...
            catch(org.omg.CORBA.SystemException corba_exception)
            {
                ...
                try
                {
        	    descType =  ldap_actual_ServiceTypeRepository.describe_type(name);
                }
                catch(org.omg.CORBA.COMM_FAILURE cf1)
                {
                  Logger.debug(this.getClass().getName() + " COMM_FAILURE Exception: " +cf1.toString());
