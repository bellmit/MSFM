 ...
 *
 * FILE GENERATED FROM KeyValueMap.template where keytype = (long) and valtype = (int)
 *
 */
...
public class LongIntMap
{
    ...
    public LongIntMap(int numberBuckets)
    {
        this(numberBuckets, DEFAULT_MODE);
    }

    public LongIntMap(int numberBuckets, int mapParameters)
    {
    ...
    public LongIntMap setValueNotFound(int ValueNotFound)
    {
    ...
    public boolean containsKey(long key, LongIntMapPolicy policy)
    {
    ...
    public boolean containsValue(int value, LongIntMapPolicy policy)
    {
        boolean contains = false;
    ...
    public LongArrayHolder getKeysForValue(int value, LongArrayHolder arrayHolder)
    {
        for (int bucketIndex = 0; bucketIndex <= highestBucketIndex; bucketIndex++)
        {
    ...
    public LongArrayHolder getKeysForValue(int value, LongArrayHolder arrayHolder, LongIntMapPolicy policy)
    {
        for (int bucketIndex = 0; bucketIndex <= highestBucketIndex; bucketIndex++)
        {
    ...
    public int get(long key, LongIntMapPolicy policy)
    {
    ...
    public int put(long key, int value, LongIntMapPolicy policy)
    {
        return buckets[(int) (key & highestBucketIndex)].put(key, value, policy);
    }

    public int put(long key, int value, LongIntMapModifyPolicy policy)
    {
        return buckets[(int) (key & highestBucketIndex)].put(key, value, policy);
    }
    ...
    public int remove(long key, LongIntMapPolicy policy)
    {
        return buckets[(int) (key & highestBucketIndex)].remove(key, policy);
    }

    public void remove(LongIntMapPolicy policy)
    {
        for (int bucketIndex = 0; bucketIndex <= highestBucketIndex; bucketIndex++)
        {
    ...
    public void remove(LongIntMapArrayHolder arrayHolder, LongIntMapPolicy policy)
    {
        for (int bucketIndex = 0; bucketIndex <= highestBucketIndex; bucketIndex++)
        {
    ...
    public void find(LongIntMapArrayHolder arrayHolder)
    {
        for (int bucketIndex = 0; bucketIndex <= highestBucketIndex; bucketIndex++)
        {
    ...
    public void find(LongIntMapArrayHolder arrayHolder, LongIntMapPolicy policy)
    {
        for (int bucketIndex = 0; bucketIndex <= highestBucketIndex; bucketIndex++)
        {
...
// bucket interface
    public interface Bucket
    {
        ...
        public int get(long key);
        public int get(long key, LongIntMapPolicy policy);
        public void getKeysForValue(int value, LongArrayHolder arrayHolder);
        public void getKeysForValue(int value, LongArrayHolder arrayHolder, LongIntMapPolicy policy);
        public int put(long key, int value);
        public int put(long key, int value, LongIntMapPolicy policy);
        public int put(long key, int value, LongIntMapModifyPolicy policy);
        public int remove(long key);
        ...
        public void remove(LongIntMapPolicy policy);
        public void remove(LongIntMapArrayHolder arrayHolder, LongIntMapPolicy policy);
        public void acceptVisitor(LongIntMapVisitor visitor);
        public void clear();
        public void find(LongIntMapArrayHolder arrayHolder);
        public void find(LongIntMapArrayHolder arrayHolder, LongIntMapPolicy policy);
    }
// unsynchronized bucket 
    protected class UnsynchronizedBucket implements Bucket
    {
        ...
        public void acceptVisitor(LongIntMapVisitor visitor)
        {
            for (int keyIndex = 0; keyIndex < keyCount; keyIndex++)
            {
                if (visitor.visit(keys[keyIndex], values[keyIndex]) == LongIntMapVisitor.ABORT)
                {
                    break;
                }
            }
...
// synchronized bucket
    protected class SynchronizedBucket implements Bucket
    {
        ...
        public synchronized boolean containsKey(long key, LongIntMapPolicy policy)
        {
            int keyIndex = keyFind(key);
            if (keyIndex >= 0)
            {
        ...
        public synchronized boolean containsValue(int value, LongIntMapPolicy policy)
        {
            for (int keyIndex = 0; keyIndex < keyCount; keyIndex++)
            {
                if (value == values[keyIndex])
        ...
        public synchronized int get(long key, LongIntMapPolicy policy)
        {
            if (keyCount > 0)
            {
                int keyIndex = keyFind(key);
        ...
        public synchronized void getKeysForValue(int value, LongArrayHolder arrayHolder)
        {
            for (int keyIndex = 0; keyIndex < keyCount; keyIndex++)
            {
                if (value == values[keyIndex])
        ...
        public synchronized void getKeysForValue(int value, LongArrayHolder arrayHolder, LongIntMapPolicy policy)
        {
            for (int keyIndex = 0; keyIndex < keyCount; keyIndex++)
            {
                if (value == values[keyIndex])
        ...
        public synchronized int put(long key, int value, LongIntMapPolicy policy)
        {
            if (keyCount == 0)
            {
                if (!policy.canInsert(key, value))
        ...
        public synchronized int put(long key, int value, LongIntMapModifyPolicy policy)
        {
            if (keyCount == 0)
            {
                if (!policy.canInsert(key, value))
        ...
        public synchronized int remove(long key, LongIntMapPolicy policy)
        {
            if (keyCount > 0)
            {
                int keyIndex = keyFind(key);
        ...
        public synchronized void remove(LongIntMapPolicy policy)
        {
            if (keyCount > 0)
            {
                int startKeyCount = keyCount;
        ...
        public synchronized void remove(LongIntMapArrayHolder arrayHolder, LongIntMapPolicy policy)
        {
            if (keyCount > 0)
            {
                int startKeyCount = keyCount;
        ...
        public synchronized void acceptVisitor(LongIntMapVisitor visitor)
        {
            for (int keyIndex = 0; keyIndex < keyCount; keyIndex++)
            {
                if (visitor.visit(keys[keyIndex], values[keyIndex]) == LongIntMapVisitor.ABORT)
        ...
        public synchronized void find(LongIntMapArrayHolder arrayHolder)
        {
            if (keyCount > 0)
            {
                arrayHolder.add(keys, values, keyCount);
        ...
        public synchronized void find(LongIntMapArrayHolder arrayHolder, LongIntMapPolicy policy)
        {
            if (keyCount > 0)
            {
                long key;
