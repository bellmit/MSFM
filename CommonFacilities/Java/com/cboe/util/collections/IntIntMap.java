 ...
 *
 * FILE GENERATED FROM KeyValueMap.template where keytype = (int) and valtype = (int)
 *
 */
...
public class IntIntMap
{
    ...
    public IntIntMap(int numberBuckets)
    {
        this(numberBuckets, DEFAULT_MODE);
    }

    public IntIntMap(int numberBuckets, int mapParameters)
    {
    ...
    public boolean containsKey(int key, IntIntMapPolicy policy)
    {
    ...
    public boolean containsValue(int value, IntIntMapPolicy policy)
    {
        boolean contains = false;
    ...
    public IntArrayHolder getKeysForValue(int value, IntArrayHolder arrayHolder)
    {
        for (int bucketIndex = 0; bucketIndex <= highestBucketIndex; bucketIndex++)
        {
    ...
    public IntArrayHolder getKeysForValue(int value, IntArrayHolder arrayHolder, IntIntMapPolicy policy)
    {
        for (int bucketIndex = 0; bucketIndex <= highestBucketIndex; bucketIndex++)
        {
    ...
    public int get(int key, IntIntMapPolicy policy)
    {
    ...
    public int put(int key, int value, IntIntMapPolicy policy)
    {
        return buckets[(int) (key & highestBucketIndex)].put(key, value, policy);
    }

    public int put(int key, int value, IntIntMapModifyPolicy policy)
    {
        return buckets[(int) (key & highestBucketIndex)].put(key, value, policy);
    }
    ...
    public int remove(int key, IntIntMapPolicy policy)
    {
        return buckets[(int) (key & highestBucketIndex)].remove(key, policy);
    }

    public void remove(IntIntMapPolicy policy)
    {
        for (int bucketIndex = 0; bucketIndex <= highestBucketIndex; bucketIndex++)
        {
    ...
    public void remove(IntIntMapArrayHolder arrayHolder, IntIntMapPolicy policy)
    {
        for (int bucketIndex = 0; bucketIndex <= highestBucketIndex; bucketIndex++)
        {
    ...
    public void find(IntIntMapArrayHolder arrayHolder)
    {
        for (int bucketIndex = 0; bucketIndex <= highestBucketIndex; bucketIndex++)
        {
    ...
    public void find(IntIntMapArrayHolder arrayHolder, IntIntMapPolicy policy)
    {
        for (int bucketIndex = 0; bucketIndex <= highestBucketIndex; bucketIndex++)
        {
...
// bucket interface
    public interface Bucket
    {
        ...
        public int get(int key);
        public int get(int key, IntIntMapPolicy policy);
        public void getKeysForValue(int value, IntArrayHolder arrayHolder);
        public void getKeysForValue(int value, IntArrayHolder arrayHolder, IntIntMapPolicy policy);
        public int put(int key, int value);
        public int put(int key, int value, IntIntMapPolicy policy);
        public int put(int key, int value, IntIntMapModifyPolicy policy);
        public int remove(int key);
        ...
        public void remove(IntIntMapPolicy policy);
        public void remove(IntIntMapArrayHolder arrayHolder, IntIntMapPolicy policy);
        public void acceptVisitor(IntIntMapVisitor visitor);
        public void clear();
        public void find(IntIntMapArrayHolder arrayHolder);
        public void find(IntIntMapArrayHolder arrayHolder, IntIntMapPolicy policy);
    }
// unsynchronized bucket 
    protected class UnsynchronizedBucket implements Bucket
    {
        ...
        public void acceptVisitor(IntIntMapVisitor visitor)
        {
            for (int keyIndex = 0; keyIndex < keyCount; keyIndex++)
            {
                if (visitor.visit(keys[keyIndex], values[keyIndex]) == IntIntMapVisitor.ABORT)
                {
                    break;
                }
            }
...
// synchronized bucket
    protected class SynchronizedBucket implements Bucket
    {
        ...
        public synchronized boolean containsKey(int key, IntIntMapPolicy policy)
        {
            int keyIndex = keyFind(key);
            if (keyIndex >= 0)
            {
        ...
        public synchronized boolean containsValue(int value, IntIntMapPolicy policy)
        {
            for (int keyIndex = 0; keyIndex < keyCount; keyIndex++)
            {
                if (value == values[keyIndex])
        ...
        public synchronized int get(int key, IntIntMapPolicy policy)
        {
            if (keyCount > 0)
            {
                int keyIndex = keyFind(key);
        ...
        public synchronized void getKeysForValue(int value, IntArrayHolder arrayHolder)
        {
            for (int keyIndex = 0; keyIndex < keyCount; keyIndex++)
            {
                if (value == values[keyIndex])
        ...
        public synchronized void getKeysForValue(int value, IntArrayHolder arrayHolder, IntIntMapPolicy policy)
        {
            for (int keyIndex = 0; keyIndex < keyCount; keyIndex++)
            {
                if (value == values[keyIndex])
        ...
        public synchronized int put(int key, int value, IntIntMapPolicy policy)
        {
            if (keyCount == 0)
            {
                if (!policy.canInsert(key, value))
        ...
        public synchronized int put(int key, int value, IntIntMapModifyPolicy policy)
        {
            if (keyCount == 0)
            {
                if (!policy.canInsert(key, value))
        ...
        public synchronized int remove(int key, IntIntMapPolicy policy)
        {
            if (keyCount > 0)
            {
                int keyIndex = keyFind(key);
        ...
        public synchronized void remove(IntIntMapPolicy policy)
        {
            if (keyCount > 0)
            {
                int startKeyCount = keyCount;
        ...
        public synchronized void remove(IntIntMapArrayHolder arrayHolder, IntIntMapPolicy policy)
        {
            if (keyCount > 0)
            {
                int startKeyCount = keyCount;
        ...
        public synchronized void acceptVisitor(IntIntMapVisitor visitor)
        {
            for (int keyIndex = 0; keyIndex < keyCount; keyIndex++)
            {
                if (visitor.visit(keys[keyIndex], values[keyIndex]) == IntIntMapVisitor.ABORT)
        ...
        public synchronized void find(IntIntMapArrayHolder arrayHolder)
        {
            if (keyCount > 0)
            {
                arrayHolder.add(keys, values, keyCount);
        ...
        public synchronized void find(IntIntMapArrayHolder arrayHolder, IntIntMapPolicy policy)
        {
            if (keyCount > 0)
            {
                int key;
