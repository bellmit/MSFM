 ...
 *
 * FILE GENERATED FROM KeyValueMap.template where keytype = (String) and valtype = (long)
 *
 */
...
public class StringLongMap
{
    ...
    public StringLongMap(int numberBuckets)
    {
        this(numberBuckets, DEFAULT_MODE);
    }

    public StringLongMap(int numberBuckets, int mapParameters)
    {
    ...
    public StringLongMap setValueNotFound(long ValueNotFound)
    {
    ...
    public boolean containsKey(String key, StringLongMapPolicy policy)
    {
    ...
    public boolean containsValue(long value, StringLongMapPolicy policy)
    {
        boolean contains = false;
    ...
    public StringArrayHolder getKeysForValue(long value, StringArrayHolder arrayHolder)
    {
        for (int bucketIndex = 0; bucketIndex <= highestBucketIndex; bucketIndex++)
        {
    ...
    public StringArrayHolder getKeysForValue(long value, StringArrayHolder arrayHolder, StringLongMapPolicy policy)
    {
        for (int bucketIndex = 0; bucketIndex <= highestBucketIndex; bucketIndex++)
        {
    ...
    public long get(String key, StringLongMapPolicy policy)
    {
    ...
    public long put(String key, long value, StringLongMapPolicy policy)
    {
        return buckets[(int) (key.hashCode() & highestBucketIndex)].put(key, value, policy);
    }

    public long put(String key, long value, StringLongMapModifyPolicy policy)
    {
        return buckets[(int) (key.hashCode() & highestBucketIndex)].put(key, value, policy);
    }
    ...
    public long remove(String key, StringLongMapPolicy policy)
    {
        return buckets[(int) (key.hashCode() & highestBucketIndex)].remove(key, policy);
    }

    public void remove(StringLongMapPolicy policy)
    {
        for (int bucketIndex = 0; bucketIndex <= highestBucketIndex; bucketIndex++)
        {
    ...
    public void remove(StringLongMapArrayHolder arrayHolder, StringLongMapPolicy policy)
    {
        for (int bucketIndex = 0; bucketIndex <= highestBucketIndex; bucketIndex++)
        {
    ...
    public void find(StringLongMapArrayHolder arrayHolder)
    {
        for (int bucketIndex = 0; bucketIndex <= highestBucketIndex; bucketIndex++)
        {
    ...
    public void find(StringLongMapArrayHolder arrayHolder, StringLongMapPolicy policy)
    {
        for (int bucketIndex = 0; bucketIndex <= highestBucketIndex; bucketIndex++)
        {
...
// bucket interface
    public interface Bucket
    {
        ...
        public long get(String key);
        public long get(String key, StringLongMapPolicy policy);
        public void getKeysForValue(long value, StringArrayHolder arrayHolder);
        public void getKeysForValue(long value, StringArrayHolder arrayHolder, StringLongMapPolicy policy);
        public long put(String key, long value);
        public long put(String key, long value, StringLongMapPolicy policy);
        public long put(String key, long value, StringLongMapModifyPolicy policy);
        public long remove(String key);
        ...
        public void remove(StringLongMapPolicy policy);
        public void remove(StringLongMapArrayHolder arrayHolder, StringLongMapPolicy policy);
        public void acceptVisitor(StringLongMapVisitor visitor);
        public void clear();
        public void find(StringLongMapArrayHolder arrayHolder);
        public void find(StringLongMapArrayHolder arrayHolder, StringLongMapPolicy policy);
    }
// unsynchronized bucket 
    protected class UnsynchronizedBucket implements Bucket
    {
        ...
        public void acceptVisitor(StringLongMapVisitor visitor)
        {
            for (int keyIndex = 0; keyIndex < keyCount; keyIndex++)
            {
                if (visitor.visit(keys[keyIndex], values[keyIndex]) == StringLongMapVisitor.ABORT)
                {
                    break;
                }
            }
...
// synchronized bucket
    protected class SynchronizedBucket implements Bucket
    {
        ...
        public synchronized boolean containsKey(String key, StringLongMapPolicy policy)
        {
            int keyIndex = keyFind(key);
            if (keyIndex >= 0)
            {
        ...
        public synchronized boolean containsValue(long value, StringLongMapPolicy policy)
        {
            for (int keyIndex = 0; keyIndex < keyCount; keyIndex++)
            {
                if (value == values[keyIndex])
        ...
        public synchronized long get(String key, StringLongMapPolicy policy)
        {
            if (keyCount > 0)
            {
                int keyIndex = keyFind(key);
        ...
        public synchronized void getKeysForValue(long value, StringArrayHolder arrayHolder)
        {
            for (int keyIndex = 0; keyIndex < keyCount; keyIndex++)
            {
                if (value == values[keyIndex])
        ...
        public synchronized void getKeysForValue(long value, StringArrayHolder arrayHolder, StringLongMapPolicy policy)
        {
            for (int keyIndex = 0; keyIndex < keyCount; keyIndex++)
            {
                if (value == values[keyIndex])
        ...
        public synchronized long put(String key, long value, StringLongMapPolicy policy)
        {
            if (keyCount == 0)
            {
                if (!policy.canInsert(key, value))
        ...
        public synchronized long put(String key, long value, StringLongMapModifyPolicy policy)
        {
            if (keyCount == 0)
            {
                if (!policy.canInsert(key, value))
        ...
        public synchronized long remove(String key, StringLongMapPolicy policy)
        {
            if (keyCount > 0)
            {
                int keyIndex = keyFind(key);
        ...
        public synchronized void remove(StringLongMapPolicy policy)
        {
            if (keyCount > 0)
            {
                int startKeyCount = keyCount;
        ...
        public synchronized void remove(StringLongMapArrayHolder arrayHolder, StringLongMapPolicy policy)
        {
            if (keyCount > 0)
            {
                int startKeyCount = keyCount;
        ...
        public synchronized void acceptVisitor(StringLongMapVisitor visitor)
        {
            for (int keyIndex = 0; keyIndex < keyCount; keyIndex++)
            {
                if (visitor.visit(keys[keyIndex], values[keyIndex]) == StringLongMapVisitor.ABORT)
        ...
        public synchronized void find(StringLongMapArrayHolder arrayHolder)
        {
            if (keyCount > 0)
            {
                arrayHolder.add(keys, values, keyCount);
        ...
        public synchronized void find(StringLongMapArrayHolder arrayHolder, StringLongMapPolicy policy)
        {
            if (keyCount > 0)
            {
                String key;
