...
public class FixSessionManager implements FixSessionManagerIF
{
    ...
    public void initialize(String propertyPrefix, Properties sessionProperties) throws Exception
    {
        if(Log.isDebugOn())
        {
            Log.debug("Initialized FixSessionManager. CFIX is Event Channel Enabled.");
        }

        this.propertyPrefix    = propertyPrefix;
        this.sessionProperties = sessionProperties;

        this.fixSessionClass   =                       Class.forName(PropertiesHelper.instance().getProperty(sessionProperties, propertyPrefix, "cfix.fixSessionManager.fixSessionClass", "com.cboe.cfix.fix.fix42.session.FixSession"));
    ...
    public synchronized void sessionLogonRequest(FixSessionIF fixSession, FixMessageIF fixMessage) throws Exception
    {
        ...
        StringBuilder sb = new StringBuilder(senderCompID.length()+40);

        sb.append("session.").append(senderCompID).append(".cfix.fixSession.senderCompID");
        if (null == fixSession.getPropertiesHelper().getProperties().getProperty(sb.toString()))
        {
        ...
        sb.setLength(0);
        sb.append("session.").append(senderCompID);
        String sessionPrefix = sb.toString();
