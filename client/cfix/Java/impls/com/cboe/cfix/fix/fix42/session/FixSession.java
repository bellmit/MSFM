...
public class FixSession implements FixSessionIF, CfixUserSessionAdminConsumer, CfixMarketDataConsumer, Comparable
{
    ...
    protected FixSessionState           fixSessionState;
    ...
    protected FixSessionWriter          fixSessionWriter;
    ...
    protected final FixSessionStatePreLogon         fixSessionStatePreLogon         = new FixSessionStatePreLogon();
    protected final FixSessionStateNormalProcessing fixSessionStateNormalProcessing = new FixSessionStateNormalProcessing();
    protected final FixSessionStateDisconnect       fixSessionStateDisconnect       = new FixSessionStateDisconnect();
    ...
    public FixSessionIF initialize(FixSessionManagerIF fixSessionManager, String propertyPrefix, Properties sessionProperties)
    {
        if(Log.isDebugOn())
        {
            Log.debug("Initialized FixSession. CFIX is IEC Enabled.");
        }
    ...
    {
        String utcTimestamp = DateHelper.stringizeDateInFixUTCTimeStampFormat();
        StringBuilder sb = new StringBuilder(utcTimestamp.length()+10);
        sb.append("(").append(fixSessionInformation.getOutstandingTestRequestList().size()+1).append("/").append(fixSessionInformation.maxOutstandingTestRequests).append(") ").append(utcTimestamp);
    ...
    public void renameSession(String name, boolean isNameSession)
    {
        ...
        sb.append("[FixSessionReader<").append(name).append(">]");
        Thread.currentThread().setName(sb.toString());

        sb.setLength(0);
        sb.append("adaptiveThreadPool(").append(name).append(")");
        threadPool.setName(sb.toString());
    ...
    public void prepareToRun()
    {
        ...
        fixSessionWriter = new FixSessionWriter(this);
    ...
    protected void logonToCas(FixLogonMessage fixLogonMessage, short userLogonType) throws DataValidationException
    {
        if(Log.isDebugOn())
        {
            Log.debug("FixSession : Logging onto CAS ");
        }
        ...
        StringBuilder loginMsg = new StringBuilder(threadName.length()+userLogonStruct.userId.length()+20);
        loginMsg.append(threadName).append(" BEGIN LOGIN(").append(userLogonStruct.userId).append(")");
        Log.information(loginMsg.toString());
        ...
        loginMsg.setLength(0);
        loginMsg.append(threadName).append(" COMPLETE LOGIN(").append(userLogonStruct.userId).append(")");
        Log.information(loginMsg.toString());
    }
    ...
    protected abstract class FixSessionState
    {
    ...
    protected class FixSessionStatePreLogon extends FixSessionState
    {
        public void processMessage() throws Exception
        {
            ...
            try
            {
                fixSessionManager.sessionLogonRequest(FixSession.this, fixLogonMessage);
            }
            catch (Exception ex)
            {
                // we don't want to consume a sequence number here
            ...
            StringBuilder name = new StringBuilder(targetCompID.length()+sessionName.length()+25);
            name.append(targetCompID).append("/").append(sessionName).append("/NetworkInstrumentor");
            fixSessionInstrumentation.getNetworkConnectionInstrumentor().rename(name.toString());
            name.setLength(0);
            name.append(targetCompID).append("/").append(sessionName).append("/QueueInstrumentor");
            fixSessionWriter.fixEventChannel.getQueueInstrumentor().rename(name.toString());
            ...
            name.setLength(0);
            name.append("FixSessionWriter{").append(sessionName).append("}");
            threadPool.execute(fixSessionWriter, name.toString());

            try
            {
                fixSessionInformation.initialize(FixSession.this);
            ...
            try
            {
                ...
                cfixMarketDataQuery = cfixSessionManager.getCfixMarketDataQuery();
                cfixMarketDataQuery.setCfixMarketDataConsumer(FixSession.this);
            }
            ...
            String threadName = Thread.currentThread().getName();
            StringBuilder loggedIn = new StringBuilder(threadName.length()+20);
            loggedIn.append(threadName).append(" Session Logged In");
            Log.information(loggedIn.toString());
        }
        ...
        public String toString()
        {
            String className = ClassHelper.getClassNameFinalPortion(getClass());
            StringBuilder name = new StringBuilder(className.length()+10);
            name.append("{").append(className).append("(???)}");
            return name.toString();
        }
    }

    protected class FixSessionStateNormalProcessing extends FixSessionState
    {
        public void processMessage() throws Exception
        {
            // get the next message from the socket -- if it has problems, then kick it upstairs,
            ...
            if (fixJustReceivedMessage.isFixSequenceResetMessage())
            {
                ...
                {
                    String threadName = Thread.currentThread().getName();
                    StringBuilder reset = new StringBuilder(threadName.length()+65);
                    reset.append(threadName).append(" RECEIVED SequenceReset-Reset from #").append(fixSessionInformation.getNextReceiveMsgSeqNum())
                         .append(" to #").append(newSeqNum);
                    Log.information(reset.toString());
    ...
    protected class FixSessionStateDisconnect extends FixSessionState
    {
        public void processMessage() throws Exception
        {
            if (disconnectReason == 0)
    ...
    protected void processInboundMessage() throws Exception
    {
        ...
        if (fixJustReceivedMessage.isFixMarketDataRequestMessage())
        {
            ...
            FixMarketDataFutureExecution marketDataFutureExecution;
            ...
            if (fixMarketDataRequestMessage.fieldSubscriptionRequestType.isDisablePrevious())
            {
                ...
                marketDataFutureExecution = (FixMarketDataFutureExecution) marketDataFutureExecutionMap.remove(mdReqID);
                ...
                if (BitHelper.isBitMaskSet(marketDataFutureExecution.getStatusBits(), FutureExecutionIF.MAIN_STATE_FINISHED))
                {
                    ...
                    if (BitHelper.isBitMaskSet(marketDataFutureExecution.getStatusBits(), FutureExecutionIF.CURRENT_STATE_SUCCEEDED))
                    {
                        ...
                        marketDataFutureExecution = new FixMarketDataFutureExecution(this, marketDataFutureExecution);
            ...
            marketDataFutureExecution = (FixMarketDataFutureExecution) marketDataFutureExecutionMap.get(mdReqID);
            if (marketDataFutureExecution != null)
            {
            ...
            marketDataFutureExecution = new FixMarketDataFutureExecution(this, fixMarketDataRequestMessage);
    ...
    /* package protected */ void cleanup()
    {
        ...
        try
        {
            if (cfixMarketDataQuery != null)
            {
                ...
                cfixMarketDataQuery.unsubscribeListener();
            }
        }
        catch (Exception ex)
        {
