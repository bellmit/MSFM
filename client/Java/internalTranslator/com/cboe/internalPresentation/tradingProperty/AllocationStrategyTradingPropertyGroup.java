//
// -----------------------------------------------------------------------------------
// Source file: AllocationStrategyTradingPropertyGroup.java
//
...
/**
 * Provides a default OldTradingPropertyGroup implementation for AllocationStrategyStructV2's.
 */
public class AllocationStrategyTradingPropertyGroup extends AbstractOldTradingPropertyGroup
{
    public static final TradingPropertyType TRADING_PROPERTY_TYPE = TradingPropertyTypeImpl.ALLOCATION_STRATEGY;
     ...
     */
    public AllocationStrategyTradingPropertyGroup(String sessionName, int classKey)
    {
     ...
     */
    public static AllocationStrategyTradingPropertyGroup getGroup(String sessionName, int classKey)
                   ...
                   InvocationTargetException, AuthorizationException, TransactionFailedException
    {
        ...
        AllocationStrategyTradingPropertyGroup castedGroup = (AllocationStrategyTradingPropertyGroup) myGroup;
        return castedGroup;
    }
    ...
    {
        AllocationStrategyTradingProperty newTP = new AllocationStrategyTradingProperty(sessionName, classKey);
        return newTP;
    }
    ...
    public AllocationStrategyTradingProperty[] getAllocationStrategyTradingProperties()
    {
        TradingProperty[] allTPs = getAllTradingProperties();
        AllocationStrategyTradingProperty[] castedTPs = new AllocationStrategyTradingProperty[allTPs.length];
    ...
    {
        return new Short(((AllocationStrategyTradingProperty)tradingProperty).getAllocationTradeType());
    }

    protected Object getSaveParmsData(TradingProperty[] allTradingProperties, Class dataType)
    {
        AllocationStrategyStructV2[] structs = new AllocationStrategyStructV2[allTradingProperties.length];
        for(int i = 0; i < allTradingProperties.length; i++)
        {
            AllocationStrategyTradingProperty allocationStrategyTradingProperty =
                    (AllocationStrategyTradingProperty) allTradingProperties[i];
            structs[i] = allocationStrategyTradingProperty.getAllocationStrategyStruct();
        }
    ...
    {
        AllocationStrategyStructV2[] allocStructs = (AllocationStrategyStructV2[]) returnValue;

        for(int i = 0; i < allocStructs.length; i++)
        {
            AllocationStrategyStructV2 allocationStrategyStruct = allocStructs[i];
            AllocationStrategyTradingProperty newTP =
                    (AllocationStrategyTradingProperty) createNewTradingProperty(getSessionName(), getClassKey());
            newTP.setAllocationStrategyStruct(allocationStrategyStruct);
            ...
            throws DataValidationException, InvocationTargetException
    {
        ...
        AllocationStrategiesClassStruct[] structs = (AllocationStrategiesClassStruct[]) returnValue;

        int classKey;

        for(int i = 0; i < structs.length; i++)
        {
            AllocationStrategiesClassStruct arrayElement = structs[i];
            classKey = getClassKeyFromStructObject(arrayElement);

            TradingPropertyFactory factory = TradingPropertyFactoryHome.find();

            AllocationStrategyTradingPropertyGroup newTPGroup =
                    (AllocationStrategyTradingPropertyGroup) factory.createNewTradingPropertyGroup(sessionName, classKey,
                                                                                            getTradingPropertyType().getName());
            newTPGroup.sequenceHolder.value = arrayElement.seqNum;
            newTPGroup.processDataFromLoad(arrayElement.allocationStrategies);
    ...
    {
        int classKey = ((AllocationStrategiesClassStruct) arrayElement).classKey;
        return classKey;
    }

    protected int getSequenceNumberFromStructObject(Object arrayElement)
    {
        int sequenceNumber = ((AllocationStrategiesClassStruct) arrayElement).seqNum;
