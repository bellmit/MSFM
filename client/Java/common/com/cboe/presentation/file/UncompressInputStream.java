...
public class UncompressInputStream extends FilterInputStream
{
    ...
    public static File uncompressInFileToOutFile(File inFile, String outFile)
        throws Exception
    {
        InputStream uInStrm =
            new UncompressInputStream(new FileInputStream(inFile));
        File returnFile  = new File(outFile);
        OutputStream outStrm =
            new FileOutputStream(returnFile);
        ...
        while (true)
        {
            int got = uInStrm.read(buf);
            if (got < 0)  break;
            //System.out.write(buf, 0, got);
            outStrm.write(buf, 0, got);
            tot += got;
        }
