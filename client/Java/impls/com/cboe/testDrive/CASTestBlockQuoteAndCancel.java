...
public class CASTestBlockQuoteAndCancel extends CASTestQuote
{
    protected Quote  qt;
    ...
    public CASTestBlockQuoteAndCancel(TestParameter parm, SessionManagerStructV2 sessionManagerStructV2, CASMeter casMeter, int tid)
        throws Exception
    {
            ...
            for(int X = 0; X < parm.numOfTests && !parm.threadDone; X++)
            {
                try
                {
                    ...
                    if (! parm.cancelByClass)
                    {
                        if (!quoteCacheByProduct.isEmpty())
                        {
                            if (curRatio == parm.quoteToQuoteCancelRatio)
                            {
                                    methodCalled = 'C';
                                    cancelSingleQuotes();
