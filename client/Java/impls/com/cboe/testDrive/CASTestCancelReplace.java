...
public class CASTestCancelReplace extends CASTestQuote
{
    ...
    public CASTestCancelReplace(TestParameter parm, SessionManagerStructV2 sessionManagerStructV2,
    ...
    public  void run()
    {
        ...
        System.out.println("Start of " + numOfOrders + " cancel:replace performance test.");
        System.out.println("Sending Cancel:Replace orders at a message rate of (" + msgRate + ")");
        ...
        long cumWaitTime = 0;
        int waitCnt = 0;
        System.out.println("Starting Cancel:Replace Order Loop");
...
//                    System.out.println("Entering Order: " + order.branch + " : " + order.correspondentFirm + ":" + order.branchSequenceNumber);
                    ...
                    if (tmpOrder != null)
                    {
                        casMeter.setMethodCalled(operationID, 'R');
                        CancelRequestStruct cancelOrder = new CancelRequestStruct(tmpOrder.orderStruct.orderId, tmpOrder.orderStruct.sessionNames[0],
                                    tmpOrder.orderStruct.userAssignedId, OrderCancelTypes.DESIRED_CANCEL_QUANTITY, tmpOrder.orderStruct.leavesQuantity);

                        oe.acceptOrderCancelReplaceRequest(cancelOrder, order);
                    }
                    else
                    {
