...
public class OrderCallbackC extends com.cboe.idl.cmiCallback.CMIOrderStatusConsumerPOA
{
    private CASMeter casMeter = null;
    private Vector interests = new Vector();
    private UserOrderCache[] orderCaches = null;
    private int numberOfInterests = 0;

    private int totalfills = 0;

    public OrderCallbackC(CASMeter meter)
    {
        casMeter = meter;
...
//        }
    ...
    {
        try {
              casMeter.setEndTime(Integer.valueOf(orderCancelReportStruct.cancelReport[0].userAssignedCancelId).intValue());
              casMeter.incrementCancelCount(Integer.valueOf(orderCancelReportStruct.cancelReport[0].userAssignedCancelId).intValue());
          updateOrderCache(orderCancelReportStruct.cancelledOrder);
        }
        catch (Exception e) {
            e.printStackTrace() ;
        }
...
/*
        for (Enumeration e = interests.elements(); e.hasMoreElements();)
        {
            if (((UserOrderCache)e.nextElement()).addOrder(order)) break;
        }
*/
    }
}
