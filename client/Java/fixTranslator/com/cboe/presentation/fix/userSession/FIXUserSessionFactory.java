//
// -----------------------------------------------------------------------------------
// Source file: FIXUserSessionFactory.java
//
...
/**
 *  Used for creating and caching a FIXUserSession
 */
public class FIXUserSessionFactory
{
    private static FIXUserSession session = null;
    ...
    public static synchronized void create(Class theClass)
    {
        try
        {
            Class interfaceClass = com.cboe.presentation.fix.userSession.FIXUserSession.class;

            Object newOBJ = theClass.newInstance();

            if (interfaceClass.isInstance(newOBJ))
            {
                session = (FIXUserSession) newOBJ;
                // hack to force UserSessionFactory to use the same singleton UserSession
                UserSessionFactory.create(session);

                fireInitializedEvent();
            }
            else
            {
                throw new IllegalArgumentException("FIXUserSessionFactory: Does not support interface com.cboe.presentation.fix.userSession.FIXUserSession. className = " + theClass);
            }
        }
        catch (Exception e)
        {
     ...
     */
    public static synchronized FIXUserSession findUserSession()
    {
        if (session != null)
        {
            return session;
        ...
        {
            throw new IllegalStateException("FIXUserSessionFactory: Create has not been called yet.");
        }
    }
